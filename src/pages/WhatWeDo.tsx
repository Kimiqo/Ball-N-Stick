...
export default function WhatWeDo() {
  ...
  const arms = DEFAULT_ARMS.map(arm => {
    if (arm.code === 'B&S Play' && settings.whatwedo_play_image_url) return { ...arm, img: settings.whatwedo_play_image_url };
    if (arm.code === 'B&S Media' && settings.whatwedo_media_image_url) return { ...arm, img: settings.whatwedo_media_image_url };
    if (arm.code === 'B&S Global' && settings.whatwedo_global_image_url) return { ...arm, img: settings.whatwedo_global_image_url };
    if (arm.code === 'B&S Event' && settings.whatwedo_event_image_url) return { ...arm, img: settings.whatwedo_event_image_url };
    if (arm.code === 'B&S Foundation' && settings.whatwedo_foundation_image_url) return { ...arm, img: settings.whatwedo_foundation_image_url };
    return arm;
  });
  ...
  return (
    ...
    >
      ...
      <section className="pt-32 md:pt-44 pb-16 md:pb-20">
        <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
          ...
          >
            ...
            <h1 className="font-display font-black uppercase text-[#F5F7FA] leading-[0.86] max-w-4xl" style={{ fontSize: 'clamp(2.2rem, 7.5vw, 6rem)' }}>
              Five Arms,
      ...
      <section className="pb-16 md:pb-20 bg-[#071A3D]">
        <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
          <Reveal>
            <div className="label text-[#D71920] py-10">Core Services</div>
          </Reveal>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-px bg-white/[0.05]">
            {SERVICES.map((s, i) => (
              ...
              >
                ...
                <p className="text-[#F5F7FA]/70 text-[0.82rem] leading-relaxed">{s.desc}</p>
            ...
            <div className={`grid grid-cols-1 lg:grid-cols-2 gap-10 items-center ${i % 2 !== 0 ? 'lg:[direction:rtl]' : ''}`}>
              <Reveal>
                <div className={`relative aspect-[4/3] overflow-hidden bg-[#0a1e50] ${i % 2 !== 0 ? '[direction:ltr]' : ''}`}>
                  {arm.img && <img src={arm.img} alt={arm.title} className="w-full h-full object-cover" loading="lazy" />}
              ...
              <Reveal delay={0.1}>
                <div className={i % 2 !== 0 ? '[direction:ltr]' : ''}>
                  ...
                  <p className="text-[#F5F7FA]/70 text-sm leading-relaxed mb-7">{arm.desc}</p>
      ...
      <section className="py-24 bg-[#071A3D] text-center border-t border-white/[0.05]">
        <div className="max-w-[1440px] mx-auto px-5 md:px-10 lg:px-16">
          <Reveal>
            <h2 className="font-display font-black uppercase text-[#F5F7FA] leading-[0.86] mb-8" style={{ fontSize: 'clamp(1.8rem, 6vw, 5.5rem)' }}>
